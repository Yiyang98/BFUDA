...
def data_loader(task):
    if task == 'M2U':
        ...
        ordinary_train_dataset = dsets.MNIST(root='/data/yiyanzha/cc/transfer/data/MNIST', train=True, transform=M_transform, download=True)
        ...
        test_dataset = USPS('/data/yiyanzha/cc/transfer/data/USPS2/Test',train=False, download=True, transform=U_transform)
        ordinary_train_dataset2 = USPS('/data/yiyanzha/cc/transfer/data/USPS2/train', train=True, download=True, transform=U_transform)
    elif task == 'U2M':
        #M1U1
        U_transform = transforms.Compose([transforms.Resize([28, 28]),transforms.ToTensor(),transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))])
        ordinary_train_dataset = USPS('/data/yiyanzha/cc/transfer/data/USPS2/train', train=True, download=True, transform=U_transform)
        full_train_loader = torch.utils.data.DataLoader(dataset=ordinary_train_dataset, batch_size=len(ordinary_train_dataset), shuffle=True)
        complementary_dataset, ccp = prepare_train_loaders(full_train_loader, 128, ordinary_train_dataset)

        M_transform = transforms.Compose([transforms.ToTensor(),transforms.Normalize(mean=(0.5, 0.5, 0.5), std=(0.5, 0.5, 0.5))])
        test_dataset = dsets.MNIST(root='/data/yiyanzha/cc/transfer/data/MNIST', train=False, transform=M_transform)
        ordinary_train_dataset2 = dsets.MNIST(root='/data/yiyanzha/cc/transfer/data/MNIST', train=True, transform=M_transform, download=True)
    else:
        print('Wrong task!')
